  ...
  set: function (arg) {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      if (lines[i].startsWith("#")) {
        ...
        let metadataRegex = new RegExp("# ([^=]*) = (.*)", "g");
        let matchedMetadata = metadataRegex.exec(lines[i]);

        // allows singly, intead of key = value
        if (!matchedMetadata) {
          metadataRegex = new RegExp("# (.*)", "g");
          matchedMetadata = metadataRegex.exec(lines[i]);

          if (matchedMetadata) {
            // sets key value pair (singly -- key, but no value)
            this.metadata.push({
              key: matchedMetadata[1],
              value: "",
            });
          }
        } else {
